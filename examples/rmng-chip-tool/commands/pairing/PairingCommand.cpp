...
CHIP_ERROR PairingCommand::RunCommand()
{
    ...
    if (mPairingMode == PairingMode::None || mPairingMode == PairingMode::CodePaseOnly)
    {
        return RunInternal(mNodeId);
    }

    CHIP_ERROR err = mCredIssuerCmds->PrepareCommissioning(mNodeId, [this](CHIP_ERROR prepareError) {
        if (prepareError != CHIP_NO_ERROR)
        {
            SetCommandExitStatus(prepareError);
            return;
        }

        CHIP_ERROR runError = RunInternal(mNodeId);
        if (runError != CHIP_NO_ERROR)
        {
            SetCommandExitStatus(runError);
        }
    });
...
void PairingCommand::OnCommissioningComplete(NodeId nodeId, CHIP_ERROR err)
{
    ...
    CHIP_ERROR finishError = mCredIssuerCmds->FinishCommissioning(
        CurrentCommissioner(), nodeId, [this](CHIP_ERROR completionError) { SetCommandExitStatus(completionError); });
    if (finishError == CHIP_ERROR_NOT_IMPLEMENTED)
    {
        SetCommandExitStatus(CHIP_NO_ERROR);
    }
    else if (finishError != CHIP_NO_ERROR)
    {
        SetCommandExitStatus(finishError);
    }
}
