...
CHIP_ERROR CHIPCommand::InitializeCommissioner(CommissionerIdentity & identity, chip::FabricId fabricId)
{
    ...
    {
        // Initialize Group Data, including IPK. Credential issuers backed by
        // an external fabric service can provide the real fabric IPK here.
        ...
        CHIP_ERROR groupDataError =
            mCredIssuerCmds->ConfigureGroupData(&sGroupDataProvider, fabricIndex, compressed_fabric_id_span);
        if (groupDataError == CHIP_ERROR_NOT_IMPLEMENTED)
        {
        ...
        {
            ReturnLogErrorOnFailure(groupDataError);
        }
    }
