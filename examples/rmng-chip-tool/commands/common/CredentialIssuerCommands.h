...
{
public:
    using CommissioningCallback = std::function<void(CHIP_ERROR)>;
    ...
    /**
     * Allow a credential issuer to replace chip-tool's example group-data setup.
     * Returning CHIP_ERROR_NOT_IMPLEMENTED preserves the legacy test IPK setup.
     */
    virtual CHIP_ERROR ConfigureGroupData(chip::Credentials::GroupDataProvider * provider, chip::FabricIndex fabricIndex,
                                          chip::ByteSpan compressedFabricId)
    {
        return CHIP_ERROR_NOT_IMPLEMENTED;
    }

    /**
     * Optional commissioning lifecycle hooks. Implementations returning
     * CHIP_ERROR_NOT_IMPLEMENTED retain chip-tool's existing behavior. A
     * handled hook must invoke completion on the Matter thread.
     */
    virtual CHIP_ERROR PrepareCommissioning(chip::NodeId nodeId, CommissioningCallback completion)
    {
        return CHIP_ERROR_NOT_IMPLEMENTED;
    }

    virtual CHIP_ERROR FinishCommissioning(chip::Controller::DeviceCommissioner & commissioner, chip::NodeId nodeId,
